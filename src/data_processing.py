...
def preprocessing_1():
    ...
    df = df[["Adj Close"]]  # Seleccion la columna "Adj Close" del DataFrame original
    df.columns = ["close"]  # Cambio del nombre de la columna a "close"
    ...
    # Obtención de la ruta completa del directorio 'data/processed' en relación al directorio actual
    current_dir = os.path.dirname(__file__)
    processed_dir = os.path.join(current_dir, '..', 'data', 'processed')

    # Creación el directorio "data/processed" si no existe
    if not os.path.exists(processed_dir):
        os.makedirs(processed_dir)

    # Guardado del DataFrame procesado en el directorio "data/processed"
